    ...
    try {
      return await this.achievementRepo.withTransaction(async (prismaTx) => {
        ...
        const initData: CreateAchievementBodyType = {
          ...
          achievementTierType: data.achievementTierType || null,
          conditionType: data.conditionType,
          conditionValue: data.conditionValue,
          conditionMeta: AchievementType,
          rewardId: data.rewardId,
          groupId: data.groupId
        }

        const achievementGroupResult = await this.achievementRepo.create(
          {
            createdById,
            data: initData
          },
        ...
        const nameKey = `achievement.name.${achievementGroupResult.id}`
        const descriptionKey = `achievement.description.${achievementGroupResult.id}`
    ...
    try {
      return await this.achievementRepo.withTransaction(async (prismaTx) => {
        ...
        const dataUpdate: UpdateAchievementBodyType = {
          ...
          conditionType: data.conditionType,
          conditionValue: data.conditionValue,
          conditionMeta: data.conditionMeta,
          rewardId: data.rewardId,
          groupId: data.groupId
        }
