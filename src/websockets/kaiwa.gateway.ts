...
const WSImpl: any = (global as any).WebSocket || (() => {
    try {
    ...
    async handleUserAudioChunk(
        ...
        try {
            ...
            if (!upstream || upstream.readyState !== (WSImpl?.OPEN ?? 1)) {
                ...
                if (!apiKey) {
                    this.logger.error('Missing GEMINI_API_KEY/GEMINI_FLASH_API_KEY for Realtime API')
                    client.emit('error', { message: 'Server missing Gemini API key' })
                    return
                }
                upstream = this.createGeminiRealtimeWS(apiKey, 'gemini-2.5-flash')
                ...
                upstream.on('close', (code, reason) => {
                    this.logger.warn(`[Kaiwa] Realtime WS closed (${code}) for user ${userId}: ${reason}`)
                    if (client.data) client.data.geminiRealtime = undefined
                })
                upstream.on('error', (err) => {
                    this.logger.error(`[Kaiwa] Realtime WS error for user ${userId}: ${err.message}`)
                        ...
                        parts: [
                            {
                                inlineData: {
                                    mimeType: 'audio/pcm',
                                    data: Buffer.isBuffer(payload) ? payload.toString('base64') : Buffer.from(payload as any).toString('base64')
                                }
                            }
                        ]
                    }
